    ...
    fluidPage(
        ...
        fluidRow(
          column(
            ...
            selectizeInput(
              ...
              choices = setNames(
                c('cost', 'barrier_count', 'potential_species', 'hmarg_length', 'hfull', 'wria_number', 'owner_type_code','percent_fish_passable_code'),
      ...
      conditionalPanel(
        ...
        selectInput(
          ...
          # TODO - Pass as argument or add to r reactive function?
          choices = setNames(
            c('cost', 'potential_species', 'barrier_count', 'hmarg_length_length', 'hfull', 'wria_number', 'owner_type_code','percent_fish_passable_code'),
...
mod_Explore_server <- function(id, r){
  moduleServer( id, function(input, output, session){
    ...
    cScatterPlotVariables <- setNames(
      c('cost', 'barrier_count', 'potential_species', 'hmarg_length', 'hfull', 'wria_number', 'owner_type_code','percent_fish_passable_code'),
      nm = c('Cost', 'Downstream Barriers', 'Potential Species', 'Marginal Habitat (km)', 'Full Habitat (km)', 'WRIA', 'Owner Type','Passability')
    )

    # histogram choices
    cHistogramVariables <- setNames(
      c('cost', 'potential_species', 'barrier_count', 'hmarg_length', 'hfull', 'wria_number', 'owner_type_code','percent_fish_passable_code'),
      nm = c('Cost', 'Potential Species', 'Downstream Barriers', 'Marginal Habitat (km)', 'Full Habitat (km)', 'WRIA', 'Ownership Type','Passability')
    )

    # color choices
    cColorVariables <- setNames(
      c('none', 'cost', 'barrier_count', 'hmarg_length', 'hfull', 'wria_number', 'owner_type_code','percent_fish_passable_code'),
      nm = c('None', 'Cost', 'Downstream Barriers', 'Marginal Habitat (km)', 'Full Habitat (km)', 'WRIA', 'Ownership Type','Passability')
    )
